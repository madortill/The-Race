...
// סדר השאלות האמריקאיות
var questions = [{
        question: "מה זה קורונה?", // השאלה
        answer1: "נגיף רנ''א שבדרך כלל גורם לזיהום קל בדרכי הנשימה או מערכת העיכול", // תשובה 1
        answer2: "חידק מסוכן שעובר רק בין בעלי חיים", // תשובה 2
        answer3: "שם של כלב ידוע ברוסיה", // תשובה 3
        answer4: "אחת הנשים העשירות שחיו אי פעם. מפורסמת בגלל שהקימה עשרות בתי יתומים לנחשים", // תשובה 4
        rightAnswer: "נגיף רנ''א שבדרך כלל גורם לזיהום קל בדרכי הנשימה או מערכת העיכול", // התשובה הנכונה. יש להעתיק את התשובה
        explanation: ".הסבר הסברי שכזה. הוא מסביר בהרחבה הרבהההה פרטים מה זה קורונה" // הסבר לתשובה הנכונה
    },
    {
        question: "מהי מגפה?",
...
var NUMBER_OF_QUESTIONS = 4; // לשנות בהתאם לכמות השאלות
var NUMBER_OF_FACTS = 4; // לשנות בהתאם לכמות הידעת
var NUM_OF_NEEDED_ITEM = 20; // לשנות בהתאם לכמות הדיסקיות שרוצים
const NUM_OF_ITEMS = 10; // לשנות בהתאם לכמות הדיסקיות שרוכשים בכל פעם
const PRICE_OF_ITEM = 100; // מחיר הדיסקיות
const COINS_ADDED_WHEN_RIGHT_ANSWER = 50; // כמות הכסף לתשובה נכונה

var NEEDED_ITEM = "כיסויי דסקיות";
...
const COINS_WHEN_CHEST = 50; //זכייה בתיבת אוצר
...
function addSpecialPanels() {
    ...
    imgPortal1.attr("src", "../assets/images/monopolbus.svg");
    ...
    imgPortal2.attr("src", "../assets/images/monopolbus.svg");
...
function finished() {
    ...
    let precentLostPoints = nMistakes /nCountQuestion * 100;
    var arrScore = [];
    ...
    $("#btnYes").on("click", function (event) {
        ...
        if (!bWasIn) {
            ...
            if (nCoins >= PRICE_OF_ITEM) {
                ...
                nNeededItem = nNeededItem + NUM_OF_ITEMS;
                updateScore();
